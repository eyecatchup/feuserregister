...
class tx_feuserregister_validator_Length extends tx_feuserregister_AbstractValidator {
	...
	public function validate() {
		...
		if ($this->_options['min']) {
			return (strlen($this->_value) >= $this->_options['min']) ? true : false;
		}
		if ($this->_options['max']) {
			return (strlen($this->_value) <= $this->_options['max']) ? true : false;
		}
	}
}

if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/feuserregister/classes/validators/class.tx_feuserregister_validator_length.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/feuserregister/classes/validators/class.tx_feuserregister_validator_length.php']);
}
